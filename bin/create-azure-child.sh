...
Create a k0rdent Azure child cluster deployment

Required Options:
  --cluster-name <name>             Name of the child cluster to create
  --location <region>               Azure region/location (e.g., eastus, westus2)
  --cp-instance-size <size>         Control plane instance size
  --worker-instance-size <size>     Worker node instance size
  --root-volume-size <gb>           Root volume size in GB
  --namespace <ns>                  Kubernetes namespace
  --template <name>                 Cluster template to use
  --credential <name>               Credential name to use
  --cp-number <num>                 Number of control plane nodes
  --worker-number <num>             Number of worker nodes
  --cluster-identity-name <name>    Name of the cluster identity
  --cluster-identity-namespace <ns> Namespace of the cluster identity
...
Examples:
  ...
  # Create a KOF regional cluster with labels and annotations
...
Prerequisites:
  ...
  - Azure credentials configured with setup-azure-cluster-deployment.sh

For more information, see the k0rdent documentation.
EOF
    }
