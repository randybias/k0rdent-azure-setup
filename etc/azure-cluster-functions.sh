...
validate_azure_cluster_ready() {
    ...
    # Check if Azure resources are healthy
    local resource_group="$cluster_name"
    local vm_count=$(az vm list -g "$resource_group" --query "length([?powerState=='VM running'])" -o tsv 2>/dev/null || echo "0")
    
    if [[ "$vm_count" -eq 0 ]]; then
        print_error "No running VMs found in Azure resource group: $resource_group"
        return 1
    fi
    
    print_success "Cluster validation passed: $vm_count running VMs in Azure"
    return 0
}